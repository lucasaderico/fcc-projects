...
const checkForStraights = (arr) => {
  const small1 = arr.slice(0, 4).toSorted((a, b) => a - b);
  const small2 = arr.slice(1).toSorted((a, b) => a - b);
  const large = arr.slice(0).toSorted((a, b) => a - b);

  if (checkConsecutive(small1) || checkConsecutive(small2)) {
    updateRadioOptions(3, 30);
  }

  if (checkConsecutive(large)) {
